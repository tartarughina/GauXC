...
template <typename T> using pinned_vector = std::vector<T>;
...
DeviceReplicatedLoadBalancer::DeviceReplicatedLoadBalancer(
    const DeviceReplicatedLoadBalancer &) = default;
DeviceReplicatedLoadBalancer::DeviceReplicatedLoadBalancer(
    DeviceReplicatedLoadBalancer &&) noexcept = default;
...
std::vector<int32_t> inline copy_shell_list(
    const size_t idx, const std::vector<int32_t> &counts,
  ...
  std::copy(position_list.begin() + start, position_list.begin() + end,
            shell_list.begin());
...
std::vector<XCTask> DeviceReplicatedLoadBalancer::create_local_tasks_() const {
  ...
  std::vector<XCTask> temp_tasks;
  temp_tasks.reserve(max_nbatches);
  std::vector<std::array<double, 3>> low_points;
  low_points.reserve(max_nbatches);
  std::vector<std::array<double, 3>> high_points;
  high_points.reserve(max_nbatches);
  std::vector<std::array<double, 3>> centers;
  centers.reserve(nspheres);
  std::vector<double> radii;
  radii.reserve(nspheres);
  std::vector<size_t> shell_sizes;
  shell_sizes.reserve(nspheres);
  ...
  std::vector<int32_t> pos_list_idx;
  pos_list_idx.reserve(max_nbatches);
  std::vector<size_t> nbe_vec;
  nbe_vec.reserve(max_nbatches);

  // The postion list is the largest struction so I am using pinned memory for
  ...
  data.temp_storage_device =
      util::cuda_malloc<char>(data.temp_storage_bytes); // char is 1 byte
  ...
  util::cuda_copy(nspheres * 3, data.centers_device, centers[0].data(),
                  "Centers HtoD");
  util::cuda_copy(nspheres, data.radii_device, radii.data(), "Radii HtoD");
  util::cuda_copy(nspheres, data.shell_sizes_device, shell_sizes.data(),
                  "ShellSize HtoD");
  ...
  for (size_t atom_batch = 0; atom_batch < num_atom_batch; ++atom_batch) {
    ...
    for (size_t atom_idx = 0;
         atom_idx < atBatchSz && atom_batch * atBatchSz + atom_idx < natoms;
         ++atom_idx) {
      ...
      for (size_t ibatch = 0; ibatch < nbatches; ++ibatch) {
        ...
        auto [npts, pts_b, pts_en, w_b, w_en] =
            (batcher.begin() + ibatch).range();
        auto [lo, up] =
            IntegratorXX::detail::get_box_bounds_points(pts_b, pts_en);
    ...
    util::cuda_copy(ncubes * 3, data.low_points_device, low_points[0].data(),
                    "Low points HtoD");
    util::cuda_copy(ncubes * 3, data.high_points_device, high_points[0].data(),
                    "High points HtoD");

    collision_detection(ncubes, nspheres, LD_bit, data.low_points_device,
                        data.high_points_device, data.centers_device,
                        data.radii_device, data.temp_storage_bytes,
                        data.temp_storage_device, data.collisions_device,
                        data.counts_device, master_stream);

    // Copy total number of collisions back to host to allocate result array
    int32_t total_collisions;
    util::cuda_copy(1, &total_collisions, data.counts_device + ncubes - 1,
                    "Total collisions DtoH");
    data.position_list_device = util::cuda_malloc<int32_t>(total_collisions);

    compute_position_list(ncubes, nspheres, LD_bit, data.shell_sizes_device,
                          data.collisions_device, data.counts_device,
                          data.position_list_device, data.nbe_list_device,
                          master_stream);
    ...
    util::cuda_copy(total_collisions, position_list.data(),
                    data.position_list_device, "Position List DtoH");
    util::cuda_copy(ncubes, pos_list_idx.data(), data.counts_device,
                    "Position List Idx DtoH");
    util::cuda_copy(ncubes, nbe_vec.data(), data.nbe_list_device,
                    "NBE counts DtoH");
    ...
    for (size_t atom_idx = 0;
      ...
      for (size_t ibatch = 0; ibatch < nbatches; ++ibatch) {
        ...
        if (world_rank == min_rank) {
          auto shell_list =
              std::move(copy_shell_list(idx, pos_list_idx, position_list));
  ...
  std::sort(local_work.begin(), local_work.end(), task_order);
  ...
  auto last_unique = std::unique(local_work_unique.begin(),
                                 local_work_unique.end(), task_equiv);
