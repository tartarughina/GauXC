...
std::shared_ptr<LoadBalancer> LoadBalancerHostFactory::get_shared_instance(
    std::string kernel_name, const RuntimeEnvironment &rt, const Molecule &mol,
  ...
  if (kernel_name == "REPLICATED-PETITE")
    ptr = std::make_unique<detail::PetiteHostReplicatedLoadBalancer>(rt, mol,
                                                                     mg, basis);

  if (kernel_name == "REPLICATED-FILLIN")
    ptr = std::make_unique<detail::FillInHostReplicatedLoadBalancer>(rt, mol,
                                                                     mg, basis);

  if (!ptr)
    GAUXC_GENERIC_EXCEPTION("Load Balancer Kernel Not Recognized: " +
                            kernel_name);
