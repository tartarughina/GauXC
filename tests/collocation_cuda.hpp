...
void cuda_check_collocation(const std::vector<XCDeviceTask> &tasks,
  ...
  for (int i = 0; i < tasks.size(); i++) {
    ...
    check_collocation_transpose(tasks[i].npts, tasks[i].bfn_screening.nbe,
                                ref_eval, eval.data(),
                                "IT = " + std::to_string(i) + " BF EVAL");
    ...
    if (check_hess) {
      ...
      auto npts = tasks[i].npts;
      auto nbe = tasks[i].bfn_screening.nbe;
      check_collocation_transpose(npts, nbe, ref_d2eval_xx, d2eval_xx.data(),
                                  "IT = " + std::to_string(i) + " BFXX EVAL");
      check_collocation_transpose(npts, nbe, ref_d2eval_xy, d2eval_xy.data(),
                                  "IT = " + std::to_string(i) + " BFXY EVAL");
      check_collocation_transpose(npts, nbe, ref_d2eval_xz, d2eval_xz.data(),
                                  "IT = " + std::to_string(i) + " BFXZ EVAL");
      check_collocation_transpose(npts, nbe, ref_d2eval_yy, d2eval_yy.data(),
                                  "IT = " + std::to_string(i) + " BFYY EVAL");
      check_collocation_transpose(npts, nbe, ref_d2eval_yz, d2eval_yz.data(),
                                  "IT = " + std::to_string(i) + " BFYZ EVAL");
      check_collocation_transpose(npts, nbe, ref_d2eval_zz, d2eval_zz.data(),
                                  "IT = " + std::to_string(i) + " BFZZ EVAL");
    }
  }
}

void test_cuda_collocation_masked_combined(const BasisSet<double> &basis,
  ...
  auto [shells_device, tasks] =
      populate_device_cuda(basis, ref_data, grad, false);

  const auto nshells_max = std::max_element(tasks.begin(), tasks.end(),
                                            [](const auto &a, const auto &b) {
                                              return a.bfn_screening.nshells <
                                                     b.bfn_screening.nshells;
                                            })
  ...
  if (grad)
    eval_collocation_masked_combined_deriv1(tasks.size(), npts_max, nshells_max,
                                            shells_device, tasks_device,
                                            stream);
  else
    eval_collocation_masked_combined(tasks.size(), npts_max, nshells_max,
                                     shells_device, tasks_device, stream);
  ...
  for (auto &t : tasks) {
    util::cuda_free(t.points_x, t.points_y, t.points_z,
                    t.bfn_screening.shell_offs, t.bfn_screening.shell_list,
                    t.bf);
    if (grad)
      util::cuda_free(t.dbfx, t.dbfy, t.dbfz);
  }
...
void test_cuda_collocation_shell_to_task(const BasisSet<double> &basis,
  ...
  device_queue stream(std::make_shared<util::cuda_stream>());
  auto [shells_device, tasks] =
      populate_device_cuda(basis, ref_data, grad, hess);
  ...
  std::vector<std::vector<int32_t>> shell_to_task_idx(basis.size()),
  ...
  auto *shell_to_task_device =
      util::cuda_malloc<ShellToTaskDevice>(basis.size());
  util::cuda_copy(basis.size(), shell_to_task_device, shell_to_task.data());
  util::cuda_device_sync();

  // Form angular momentum batches for collocation eval
  auto max_l = std::max_element(
                   basis.begin(), basis.end(),
                   [](const auto &a, const auto &b) { return a.l() < b.l(); })
                   ->l();
  std::vector<AngularMomentumShellToTaskBatch> l_batched_shell_to_task(max_l +
                                                                       1);
  {
    ...
    for (auto l = 0; l <= max_l; ++l) {
      ...
      size_t total_ntask = std::accumulate(
          h, h + nsh, 0ul, [](auto &a, auto &b) { return a + b.ntask; });
  ...
  if (hess)
    eval_collocation_shell_to_task_hessian(
        max_l, l_batched_shell_to_task.data(), tasks_device, stream);
  else if (grad)
    eval_collocation_shell_to_task_gradient(
        max_l, l_batched_shell_to_task.data(), tasks_device, stream);
  else
  ...
  for (auto &t : tasks) {
    ...
    if (grad)
      util::cuda_free(t.dbfx, t.dbfy, t.dbfz);
    if (hess)
      util::cuda_free(t.d2bfxx, t.d2bfxy, t.d2bfxz, t.d2bfyy, t.d2bfyz,
                      t.d2bfzz);
  }
