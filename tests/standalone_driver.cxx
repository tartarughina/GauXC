...
#define OPTIONAL_KEYWORD(NAME, VAR, TYPE)                                      \
  if (input.containsData(NAME)) {                                              \
    ...
    std::map<std::string, ExecutionSpace> exec_space_map = {
        {"HOST", ExecutionSpace::Host}, {"DEVICE", ExecutionSpace::Device}};
    ...
    if (!world_rank) {
      ...
      if (integrate_exx) {
        std::cout << "  EXX.TOL_E         = " << sn_link_settings.energy_tol
                  << std::endl
                  << "  EXX.TOL_K         = " << sn_link_settings.k_tol
                  << std::endl;
      }
      std::cout << std::endl;
    }
    ...
    auto mg = MolGridFactory::create_default_molgrid(
        mol, prune_map.at(prune_spec), BatchSize(batch_size),
    ...
    size_t N_EL_ref = MolMeta(mol).sum_atomic_charges();
    {
      ...
      if (P.rows() != P.cols())
        GAUXC_GENERIC_EXCEPTION("Density Must Be Square");
      if (P.rows() != basis.nbf())
      ...
      if (integrate_vxc) {
        try {
          ...
          if (world_rank == 0) {
            std::cout << "** Warning: Could Not Find Reference VXC"
                      << std::endl;
          }
          VXC_ref.fill(0);
        }
        if (not rks) {
          try {
            ...
            if (world_rank == 0) {
              std::cout << "** Warning: Could Not Find Reference VXCz"
                        << std::endl;
            }
            VXCz_ref.fill(0);
          }
        ...
        if (gks) {
          try {
            ...
            if (world_rank == 0) {
              std::cout << "** Warning: Could Not Find Reference VXCy/x"
                        << std::endl;
            }
            VXCy_ref.fill(0);
            VXCx_ref.fill(0);
        ...
        try {
          ...
          if (world_rank == 0) {
            std::cout << "** Warning: Could Not Find Reference EXC"
                      << std::endl;
          }
          EXC_ref = 0.;
        }
    ...
    if (functional_map.key_exists(func_spec)) {
      ...
      for (auto n : libxc_names) {
        funcs.push_back(
            {1.0, ExchCXX::XCKernel(ExchCXX::libxc_name_string(n), polar)});
      }
    ...
    XCIntegratorFactory<matrix_type> integrator_factory(
        int_exec_space, "Replicated", integrator_kernel, lwd_kernel,
        reduction_kernel);
    ...
    if (integrate_den) {
      if ((uks or gks) and !world_rank) {
        std::cout
            << "Warning: integrate_den will only integrate the scalar density!"
            << std::endl;
      }
    ...
    if (integrate_vxc) {
      if (rks) {
        ...
        std::tie(EXC, VXC, VXCz, VXCy, VXCx) =
            integrator.eval_exc_vxc(P, Pz, Py, Px);
      }
      std::cout << std::scientific << std::setprecision(12);
      if (!world_rank)
    ...
    if (integrate_exc_grad) {
      ...
      if (!world_rank) {
        ...
        for (auto iAt = 0; iAt < mol.size(); ++iAt) {
          std::cout << "  " << std::setw(16) << EXC_GRAD[3 * iAt + 0]
                    << std::setw(16) << EXC_GRAD[3 * iAt + 1] << std::setw(16)
                    << EXC_GRAD[3 * iAt + 2] << std::endl;
        }
      }
    }
    ...
    double xc_int_dur =
        std::chrono::duration<double>(xc_int_end - xc_int_start).count();
    ...
    if (!world_rank) {
      ...
      if (integrate_den) {
        ...
        std::cout << "N_EL Diff         = "
                  << std::abs(N_EL_ref - N_EL) / N_EL_ref << std::endl;
      }
    ...
    if (input.containsData("GAUXC.OUTFILE")) {
      ...
      auto dset =
          file.createDataSet<double>("/DENSITY" + ugks_scalar, mat_space);
