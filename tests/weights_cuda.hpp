...
void test_cuda_weights(std::ifstream &in_file) {
  ...
  for (auto &task : ref_data.tasks_unm) {
    ...
    weights.insert(weights.end(), task.weights.begin(), task.weights.end());

    size_t npts = task.points.size();
    dist_nearest.insert(dist_nearest.end(), npts, task.dist_nearest);
    iparent.insert(iparent.end(), npts, task.iParent);
  }

  for (auto &task : ref_data.tasks_mod) {
    weights_ref.insert(weights_ref.end(), task.weights.begin(),
                       task.weights.end());
  }
  ...
  util::cuda_copy_2d(rab_d, LDatoms * sizeof(double), rab_inv.data(),
                     natoms * sizeof(double), natoms * sizeof(double), natoms,
                     "RAB H2D");
  ...
  cuda_aos_scheme1_weights_wrapper(
      npts, natoms, points_x_d, points_y_d, points_z_d, rab_d, LDatoms,
      coords_d, dist_scr_d, LDatoms, iparent_d, distnea_d, weights_d, stream);

  util::cuda_device_sync();
  util::cuda_copy(npts, weights.data(), weights_d);
  util::cuda_free(points_x_d, points_y_d, points_z_d, weights_d, iparent_d,
                  distnea_d, rab_d, coords_d, dist_scr_d);
